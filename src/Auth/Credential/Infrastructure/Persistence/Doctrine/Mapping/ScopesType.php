...
final class ScopesType extends JsonType
{
    ...
    public function convertToDatabaseValue($value, AbstractPlatform $platform): string
    {
        if (!$value instanceof Scopes) {
            $message = sprintf('In class %s the values must values of the class %s', __CLASS__, Scopes::class);
    ...
    public function convertToPHPValue($value, AbstractPlatform $platform): Scopes
    {
        ...
        try {
            ...
            return Scopes::fromArray($result);
        } catch (\JsonException $e) {
